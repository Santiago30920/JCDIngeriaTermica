...
if (!isset($_SESSION['Cedula'])) {
    ...
    $id = (int)$_GET['id1'];
    $setencia = $bd->query("SELECT * FROM preventivo WHERE idEquipo = ". $id. ";");
    //$setencia->fetchAll();
...
<body>
    ...
    <div class="registro">
        <h2>Preventivo</h2>
        <form action="../Dao/editarPreventivo.php" class="Registrar" method="POST">
        <select name="txtMant" id="mant" class="Rol-50" style="width:84%" disabled>
        <?php
            foreach($equipos as $dato){?>
            <option value="<?php echo $dato->idEquipo?>"><?php echo $dato->idEquipo?></option>
                <?php } 
            ?>
            </select>
            ...
            <input type="hidden" name="id2" value="<?php echo $dato->idPreventivo;?>">
            <?php }?>
            <br>
